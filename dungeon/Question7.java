...
public class Question7 {
	public static void main(String[] args) {
		...
		while (numberOfPits-- > 0) {
			...
			int pitColumn = sc.nextInt();

			grid[pitRow - 1][pitColumn - 1] = -2;
		}

		int adventureMinimumSteps = findAdventureMinimumSteps(adventureRow, adventureCol, grid, false, goldRow,
				goldCol);
		int monsterMinimumSteps = findAdventureMinimumSteps(monsterRow, monsterColumn, grid, true, goldRow, goldCol);

		if (monsterMinimumSteps >= adventureMinimumSteps)
			System.out.println("Minumum number of steps: " + adventureMinimumSteps);
		else {
			System.out.println("Through Trigger");
			int adventureMinimumStepsForTrigger = findAdventureMinimumSteps(adventureRow, adventureCol, grid, false,
					triggerRow, triggerColumn);

			int triggerToGoldMinimumSteps = findAdventureMinimumSteps(triggerRow, triggerColumn, grid, false, goldRow,
					goldCol);

			System.out.println(
					"Minimum number of steps:" + (triggerToGoldMinimumSteps + adventureMinimumStepsForTrigger));
		}
